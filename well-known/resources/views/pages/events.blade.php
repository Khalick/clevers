...
@section('content')
    <div class="row d-flex justify-content-evenly mb-4">
        ...
        <div class="col-md-12 col-4 text-end">
            ...
            <a class="btn btn-danger" href="/events">
               Clear Search
            </a>
            @endif
            <a  class="btn btn-danger" href="/events/new">New Event</a>
        </div>
    </div>
    <div class="row">
<div class="dt-responsive table-responsive">
    <table id="cbtn-selectors" class="table table-striped nowrap">
        ...
        <tbody>
        @php
            $courses=\App\Models\Event::query();
 ...
 @endphp
        ...
        @foreach($courses as $course)
            <tr>
                ...
                <td>
                    {{date('H:i',strtotime($course->startTime))}}
                </td>
                <td>
                    {{date('H:i',strtotime($course->endTime))}}
                </td>
                <td>
                    {{$course->title}}
                </td>
                ...
                <td>
                    @if($course->status=='Active')
                        <a href="/events/deactivate/{{$course->id}}" class="btn btn-warning btn-sm">Deactivate</a>
                    @else
                        <a href="/events/activate/{{$course->id}}" class="btn btn-primary btn-sm">Activate</a>
                    @endif
                        <a href="/events/delete/{{$course->id}}" class="btn btn-danger btn-sm">Delete</a>
                </td>
            </tr>
        @endforeach
        </tbody>
