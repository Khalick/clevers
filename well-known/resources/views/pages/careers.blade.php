...
@section('content')
    <div class="row d-flex justify-content-evenly mb-4">
        ...
        <div class="col-md-12 col-4 text-end">
            ...
            <a class="btn btn-danger" href="/careers">
               Clear Search
            </a>
            @endif
            <a  class="btn btn-danger" href="/careers/new">New Opening</a>
        </div>
    </div>
    <div class="row">
<div class="dt-responsive table-responsive">
    <table id="cbtn-selectors" class="table table-striped nowrap">
        ...
        <tbody>
        @php
            $courses=\App\Models\Career::query();
            if (!empty($_GET['search'])){
                $courses=$courses->where('title','like','%'.$_GET['search'].'%')
                ->orWhere('requirements','like','%'.$_GET['search'].'%')
 ...
 @endphp
        ...
        @foreach($courses as $course)
            <tr>
                ...
                <td>
                    {{date('d/m/Y H:i',strtotime($course->applyBy))}}
                </td>
                <td>
{{\App\Models\JobApplication::where('jobId',$course->id)->count()}}
                </td>
                <td>
                  Ksh.{{$course->fees}}
                </td>
                ...
                <td>
                       <a href="/careers/delete/{{$course->id}}" class="btn btn-danger btn-sm">Delete</a>
                </td>
            </tr>
        @endforeach
        </tbody>
